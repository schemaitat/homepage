...
toc: false
tags: ["tutorial", "linux", "docker", "zsh", "development", "vs-code"]
...
The script is based on [zsh-in-docker](https://github.com/deluan/zsh-in-docker) and contains some more configuration paramters like adding dotfiles and executing a post script. The script works like a cli and uses the bash `getopts` functionality to parse the arguments.
...
The parameter `-s` accepts either a local file or an url whereas the `-d` only accepts urls right now and tries tu curl them.
